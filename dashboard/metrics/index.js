...
var servers = 
[
	{name: "computer",url:`http://${ip}:3000/`, status: "#cccccc", scoreTrend : []},
...
function updateHealth(server)
{
	...
	// Update score calculation.
        //This means the http get request has failed
        if((server.latency == 5000) || (server.statusCode != 200))
        {
               score = 1
        }
        if (server.statusCode){
        	if(server.cpu <= 1 && server.latency <= 10 && server.statusCode == 200 && server.memoryLoad <= 27)
        	{	  
               		score = 4;
        	}
        	else if(server.cpu <= 1 && (server.latency > 10 && server.latency <=22) && server.statusCode == 200 && server.memoryLoad <= 27)
        	{
               		score = 3;
        	}
        	else if((server.cpu > 1 && server.cpu <=8) && server.latency <= 10 && server.memoryLoad <= 27 && server.statusCode == 200)
        	{
               		score = 3;
        	}
        	else if(server.cpu <= 1 && server.latency <= 10 && (server.memoryLoad > 27 && server.memoryLoad <= 56) && server.statusCode == 200)
        	{
               		score = 3;
        	}
        	else if(server.cpu <=1 && (server.latency > 10 && server.latency <=22) && (server.memoryLoad > 27 && server.memoryLoad <= 56) && server.statusCode == 200)
        	{	
               		score = 3;
        	}
                else if((server.cpu > 1 && server.cpu <=8) && (server.memoryLoad > 27 && server.memoryLoad <= 56) && server.statusCode == 200)             
                {
                        score = 2;
                }
        	else if((server.cpu > 8 && server.cpu <= 46) || (server.memoryLoad >56 && server.memoryLoad<= 70 ))
        	{ 
               		score = 2;
        	}
        	else
		...
		if(server.cpu <= 1 && server.memoryLoad <= 27)
                {
                        score = 4;
                }
                else if((server.cpu > 1 && server.cpu <=8) && server.memoryLoad <= 27 )
                {
                        score = 3;
                }
                else if(server.cpu <= 1 && (server.memoryLoad > 27 && server.memoryLoad <= 56) )
                {
                        score = 3;
                }
                else if((server.cpu > 1 && server.cpu <=8) &&  (server.memoryLoad > 27 && server.memoryLoad <= 56) )
                {
                        score = 2;
                }
                else if((server.cpu > 8 && server.cpu <= 46) || (server.memoryLoad >56 && server.memoryLoad<= 70 ))
